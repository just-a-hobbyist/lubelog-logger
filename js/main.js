...
const refreshIntervalSelect = document.getElementById(
    ...
    if (!savedCreds) {
        ...
        if (!vehiclesJSON) {
            ...
            try {
                ...
                console.error(
                    "Failed to parse cached vehicle data. Refetching.",
                    e,
                );
                fetchVehicles(creds);
            }
        ...
        if (savedEntries)
            showToast(
                `${savedEntries} entr${savedEntries === 1 ? "y is" : "ies are"} saved and waiting to be submitted`,
            );
    }
