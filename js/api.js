...
async function fetchVehicles(credentials) {
    ...
    try {
        ...
        if (
            toastTimeout &&
            toast.textContent === "Refreshing Vehicle List..."
        ) {
        ...
        alert(
            `Error fetching vehicles: ${error.message}. Please check credentials and server address.`,
        );
...
async function addRecord(vehicleId, record, type, isRetry = false) {
    ...
    try {
        ...
        const response = await fetch(
            `${credentials.domain}/api/vehicle/${type}records/add?vehicleId=${vehicleId}`,
            {
                method: "POST",
                headers: {
                    Authorization: `Basic ${encodedCredentials}`,
...
function compareVersions(cur, upd) {
    const versionError = new Error(
        `Version error: either ${cur} or ${upd} is an invalid version number!`,
    );
...
async function checkForUpdates() {
    ...
    try {
        ...
        const response = await fetch(
            "https://just-a-hobbyist.github.io/lubelog-logger/version.json?t=" +
                new Date().getTime(),
        );
        ...
        if (compareVersions(currentVersion, remoteServerVersion.version)) {
            ...
            showToast(
                `Version ${remoteServerVersion.version} is available! Please update your server via 'git pull'.`,
            );
        } else {
